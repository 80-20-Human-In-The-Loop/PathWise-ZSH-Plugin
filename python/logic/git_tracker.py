...
# Import constants
from python.constants.git_tracker import (
    COMMIT_CATEGORIES,
    DEFAULT_CATEGORY,
    DEFAULT_EMOJI,
)
...
def categorize_commit(commit_message: str) -> str:
    ...
    for category, info in COMMIT_CATEGORIES.items():
        ...
        if keyword_count > 0:
            # Calculate score as keyword_count * priority
...
def get_random_keyword_suggestions(
    ...
    for i in range(min(num_suggestions, len(available_categories))):
        ...
        keyword = random.choice(info["keywords"])  # noqa: S311
...
def format_keyword_suggestion_shell(
    ...
    keywords = []
    for _category, emoji, keyword in suggestions:
...
if __name__ == "__main__":
    ...
    for commit in test_commits:
        ...
        emoji = COMMIT_CATEGORIES.get(
            category, {"priority": 0, "emoji": DEFAULT_EMOJI, "keywords": []}
        )["emoji"]
